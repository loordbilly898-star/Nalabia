...
const RedFlagDetectorView: React.FC<RedFlagDetectorViewProps> = ({
  settings,
}) => {
  ...
  const handleAnalyze = async () => {
    ...
    if (needsSubscription) {
      ...
      if (
        ...
        setErrorMsg(
          "Você atingiu o limite de segurança de 1000 requisições. Volte amanhã para continuar!",
        );
        return;
      }
    }
    ...
    try {
      ...
      setAnalysisResult({
        ghostingProbability:
          typeof data.ghostingProbability === "number"
            ? data.ghostingProbability
            : 0,
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400">
            Cole o histórico da conversa e descubra se você está prestes a tomar
            ghosting ou se ela é tóxica.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div
            className={`relative border-2 border-dashed rounded-2xl overflow-hidden transition-colors flex flex-col items-center justify-center min-h-[120px] ${
              selectedImages.length > 0
                ? `border-gold-dim/20 ${getThemeInputBg().split(" ")[0]} p-4`
                : `border-gold-dim/20 ${getThemeInputBg().split(" ")[0]} hover:border-rose-500/50 cursor-pointer`
            }`}
            onClick={() =>
              selectedImages.length === 0 && fileInputRef.current?.click()
            }
          >
            {selectedImages.length > 0 ? (
              <div className="w-full h-full flex flex-col">
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 overflow-y-auto max-h-[200px] custom-scrollbar pr-1">
                  {selectedImages.map((img, idx) => (
                    <div
                      key={idx}
                      className="relative group aspect-[3/4] rounded-lg overflow-hidden border border-gold-dim/10"
                    >
                      <img
                        src={img}
                        alt={`Upload ${idx + 1}`}
              ...
              <div className="text-center p-4">
                ...
                <p className="text-gray-300 font-medium text-sm">
                  Ou adicione prints da conversa
                </p>
              </div>
            )}
            <input
          ...
          <button
            ...
            disabled={
              (!chatHistory.trim() && selectedImages.length === 0) ||
          ...
          {analysisResult ? (
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div
                  className={`${getThemeInputBg().split(" ")[0]} border border-gold-dim/10 rounded-xl p-5 flex flex-col items-center justify-center text-center`}
                >
                  ...
                  <div
                    className={`text-4xl font-bold font-mono ${
                      analysisResult.ghostingProbability > 70
                        ? "text-rose-500"
                        : analysisResult.ghostingProbability > 40
                          ? "text-amber-500"
                          : "text-emerald-500"
                    }`}
                  >
                    {typeof analysisResult.ghostingProbability === "number" ||
                    typeof analysisResult.ghostingProbability === "string"
                      ? analysisResult.ghostingProbability
                      : JSON.stringify(analysisResult.ghostingProbability)}
                    %
                  </div>
                </div>
                <div
                ...
                >
                  ...
                  <div
                    className={`text-2xl font-bold font-mono uppercase tracking-wider ${
                      analysisResult.toxicityLevel === "Alto"
                        ? "text-rose-500"
                        : analysisResult.toxicityLevel === "Médio"
                          ? "text-amber-500"
                          : "text-emerald-500"
                    }`}
                  >
                    {typeof analysisResult.toxicityLevel === "string"
                      ? analysisResult.toxicityLevel
                      : JSON.stringify(analysisResult.toxicityLevel)}
                  </div>
                </div>
              </div>
              ...
              {/* Verdict & Advice */}
              <div
                className={`${getThemeInputBg().split(" ")[0]} border border-gold-dim/10 rounded-xl p-5 space-y-4`}
              >
                <div>
                  <h3 className="text-xs font-mono text-gold-glow uppercase tracking-widest mb-2">
                    Veredito
                  </h3>
                  <p className="text-gray-200 text-sm leading-relaxed">
                    {typeof analysisResult.verdict === "string"
                      ? analysisResult.verdict
                      : JSON.stringify(analysisResult.verdict)}
                  </p>
                </div>
                <div className="pt-4 border-t border-gold-dim/10">
                  <h3 className="text-xs font-mono text-blue-400 uppercase tracking-widest mb-2">
                    O que fazer agora
                  </h3>
                  <p className="text-gray-300 text-sm leading-relaxed">
                    {typeof analysisResult.advice === "string"
                      ? analysisResult.advice
                      : JSON.stringify(analysisResult.advice)}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  analysisResult.redFlags.length > 0 ? (
                    <ul className="space-y-2">
                      {analysisResult.redFlags.map((flag, i) => (
                        <li
                          key={i}
                          className="text-xs text-rose-100/80 flex items-start gap-2"
                        >
                ...
                <div className="bg-emerald-950/20 border border-emerald-900/30 rounded-xl p-4">
                  ...
                  {Array.isArray(analysisResult.greenFlags) &&
                  analysisResult.greenFlags.length > 0 ? (
                    <ul className="space-y-2">
                      {analysisResult.greenFlags.map((flag, i) => (
                        <li
                          key={i}
                          className="text-xs text-emerald-100/80 flex items-start gap-2"
                        >
            ...
            <div
              className={`h-full flex flex-col items-center justify-center text-center p-8 border border-dashed border-gold-dim/20 rounded-2xl ${getThemeInputBg().split(" ")[0]} opacity-50`}
            >
              ...
              <p className="text-gray-500 max-w-sm text-sm">
                Cole a conversa ao lado para a IA analisar o comportamento dela
                e detectar possíveis red flags ou ghosting iminente.
              </p>
            </div>
          )}
        </div>
