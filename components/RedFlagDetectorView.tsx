        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex-1 min-h-[200px] flex flex-col">
            <textarea
              ...
              className={`flex-1 w-full ${getThemeInputBg().split(' ')[0]} border border-nalabia-800/50 rounded-2xl p-4 text-gray-300 focus:outline-none focus:border-red-500/50 resize-none custom-scrollbar font-mono text-sm leading-relaxed`}
            />
          </div>
          ...
          <div 
            className={`relative border-2 border-dashed rounded-2xl overflow-hidden transition-colors flex flex-col items-center justify-center min-h-[120px] ${
              selectedImages.length > 0 ? `border-nalabia-800/50 ${getThemeInputBg().split(' ')[0]} p-4` : `border-nalabia-800/30 ${getThemeInputBg().split(' ')[0]} hover:border-red-500/50 cursor-pointer`
            }`}
          ...
          >
            {selectedImages.length > 0 ? (
              <div className="w-full h-full flex flex-col">
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 overflow-y-auto max-h-[200px] custom-scrollbar pr-1">
                  {selectedImages.map((img, idx) => (
                    <div key={idx} className="relative group aspect-[3/4] rounded-lg overflow-hidden border border-nalabia-800/50">
                      ...
                      <button 
                        ...
                        className="absolute top-1 right-1 p-1 bg-black/60 hover:bg-red-500/80 rounded-full text-white backdrop-blur-sm transition-colors opacity-0 group-hover:opacity-100"
                      >
                        <X size={12} />
                      </button>
                    </div>
                  ...
                  <div 
                    ...
                    className="aspect-[3/4] rounded-lg border border-dashed border-nalabia-800/50 flex flex-col items-center justify-center cursor-pointer hover:border-red-500/50 hover:bg-red-900/10 transition-colors"
                  >
                    <Upload size={20} className="text-red-500/50 mb-1" />
              ...
              <div className="text-center p-4">
                <Upload size={24} className="mx-auto mb-2 text-red-500/50" />
          ...
          <button
            ...
            className={`w-full py-4 rounded-xl font-bold tracking-widest uppercase flex items-center justify-center gap-2 transition-all ${
              (!chatHistory.trim() && selectedImages.length === 0) || status !== ProcessingState.IDLE
                ...
                : 'bg-red-900/50 text-red-400 border border-red-900/50 hover:bg-red-900/80 hover:text-white hover:shadow-[0_0_20px_rgba(239,68,68,0.2)]'
            }`}
          >
          ...
          {analysisResult ? (
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className={`${getThemeInputBg().split(' ')[0]} border border-nalabia-800/50 rounded-xl p-5 flex flex-col items-center justify-center text-center`}>
                  <h3 className="text-[10px] font-mono text-gray-500 uppercase tracking-widest mb-2">Chance de Ghosting</h3>
                  <div className={`text-4xl font-bold font-mono ${
                    analysisResult.ghostingProbability > 70 ? 'text-red-500' :
                    analysisResult.ghostingProbability > 40 ? 'text-yellow-500' : 'text-green-500'
                  }`}>
                ...
                <div className={`${getThemeInputBg().split(' ')[0]} border border-nalabia-800/50 rounded-xl p-5 flex flex-col items-center justify-center text-center`}>
                  ...
                  <div className={`text-2xl font-bold font-mono uppercase tracking-wider ${
                    analysisResult.toxicityLevel === 'Alto' ? 'text-red-500' :
                    analysisResult.toxicityLevel === 'Médio' ? 'text-yellow-500' : 'text-green-500'
                  }`}>
              ...
              {/* Verdict & Advice */}
              <div className={`${getThemeInputBg().split(' ')[0]} border border-nalabia-800/50 rounded-xl p-5 space-y-4`}>
                <div>
                  <h3 className="text-xs font-mono text-nalabia-gold uppercase tracking-widest mb-2">Veredito</h3>
                  <p className="text-gray-200 text-sm leading-relaxed">{analysisResult.verdict}</p>
                </div>
                <div className="pt-4 border-t border-nalabia-800/30">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-red-950/20 border border-red-900/30 rounded-xl p-4">
                  <h3 className="text-[10px] font-mono text-red-500 uppercase tracking-widest mb-3 flex items-center gap-2">
