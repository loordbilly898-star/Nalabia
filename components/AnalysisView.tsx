  ...
  return (
    <div className="bg-obsidian-light border border-gold-dim/20 rounded-lg p-4 space-y-4 mb-6 shadow-lg shadow-black/50 animate-fade-in">
        ...
        <div>
          <div className="flex justify-between text-[10px] font-mono uppercase text-gray-400 mb-1">
            ...
            <span className="text-emerald-400">
              {typeof analysis.interestScore === "number" ||
              typeof analysis.interestScore === "string"
          ...
          {renderProgressBar(
            typeof analysis.interestScore === "number"
              ? analysis.interestScore
              : parseInt(String(analysis.interestScore)) || 0,
            "bg-emerald-400",
          )}
        </div>
        <div>
          <div className="flex justify-between text-[10px] font-mono uppercase text-gray-400 mb-1">
            ...
            <span className="text-blue-400">
              ...
              typeof analysis.investmentScore === "string"
          ...
          {renderProgressBar(
            typeof analysis.investmentScore === "number"
              ? analysis.investmentScore
              : parseInt(String(analysis.investmentScore)) || 0,
            "bg-blue-400",
          )}
        </div>
          ...
          <div className="flex-1">
            <div className="flex justify-between text-[10px] font-mono uppercase text-gray-400 mb-1">
              ...
              <span className="text-red-400">
                ...
                typeof analysis.riskScore === "string"
                  ? analysis.riskScore
                  : 0}
                %
              </span>
            ...
            {renderProgressBar(
              typeof analysis.riskScore === "number"
                ? analysis.riskScore
                : parseInt(String(analysis.riskScore)) || 0,
              "bg-red-400",
            )}
          </div>
          <div className="flex-1">
            <div className="flex justify-between text-[10px] font-mono uppercase text-gray-400 mb-1">
              ...
              <span className="text-gold">
                {typeof analysis.meetingChance === "number" ||
                typeof analysis.meetingChance === "string"
                  ? analysis.meetingChance
                  : 0}
                %
              </span>
            </div>
            {renderProgressBar(
              typeof analysis.meetingChance === "number"
                ? analysis.meetingChance
                : parseInt(String(analysis.meetingChance)) || 0,
              "bg-gold",
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        ...
        <div className="bg-obsidian/50 p-2 rounded border border-gold-dim/10">
          ...
          <div className="text-sm font-semibold text-blue-100">
            {typeof analysis.emotion === "string"
              ? analysis.emotion
              : JSON.stringify(analysis.emotion)}
          </div>
        </div>
        ...
        <div className="bg-obsidian/50 p-2 rounded border border-gold-dim/10">
          ...
          <div className="text-xs text-gray-300">
            {typeof analysis.dynamic === "string"
              ? analysis.dynamic
              : JSON.stringify(analysis.dynamic)}
          </div>
        </div>
        ...
        <div className="bg-obsidian/50 p-2 rounded border border-gold-dim/10">
          ...
          <div className="text-sm font-bold text-gold-glow">
            {typeof analysis.rhythm === "string"
              ? analysis.rhythm
              : JSON.stringify(analysis.rhythm)}
          </div>
        </div>
      </div>
        ...
        <div className="bg-blue-900/40 border border-blue-500/50 rounded-lg p-3 flex items-start space-x-3 mb-2 animate-pulse">
          ...
          <div className="flex-1">
            <h4 className="text-[10px] font-bold text-blue-300 uppercase tracking-wider mb-1">
              Estratégia: Sem Resposta
            </h4>
            <p className="text-[11px] text-blue-100 leading-snug">
              A última mensagem identificada é sua. O NaLábia recomenda aguardar
              o movimento dela para não parecer ansioso ou reativo.
            </p>
          </div>
        </div>
      )}
          ...
          {analysis.suggestedTiming && (
            <div className="bg-obsidian/50 p-2 rounded border border-blue-500/20">
              ...
              <div className="text-xs text-blue-100">
                {typeof analysis.suggestedTiming === "string"
                  ? analysis.suggestedTiming
                  : JSON.stringify(analysis.suggestedTiming)}
              </div>
            </div>
          )}
          {analysis.errorAlert && (
      ...
      <div className="text-[10px] font-mono text-gray-600 text-right uppercase mt-2">
        Modo:{" "}
        {typeof analysis.detectedMode === "string"
          ? analysis.detectedMode
          : JSON.stringify(analysis.detectedMode)}
      </div>
    </div>
  );
};
