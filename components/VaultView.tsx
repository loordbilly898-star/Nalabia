  ...
  return (
    <div className="h-full flex flex-col p-4 md:p-6 overflow-y-auto custom-scrollbar">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400">
            Seu arsenal pessoal de abridores e respostas que funcionam.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div
          className={`flex-1 flex flex-col items-center justify-center text-center p-8 border border-dashed border-gold-dim/10 rounded-2xl ${getThemeInputBg().split(" ")[0]}`}
        >
          <Lock className="text-gray-600 mb-4" size={48} />
          <h3 className="text-xl font-bold text-white mb-2">Cofre Vazio</h3>
          <p className="text-gray-400 max-w-md">
            Você ainda não salvou nenhuma resposta. Quando a IA gerar uma
            resposta matadora, clique no ícone de salvar para guardá-la aqui.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredResponses.map((response) => (
            <div
              key={response.id}
              className={`${getThemeInputBg().split(" ")[0]} border border-gold-dim/10 rounded-xl p-5 flex flex-col group hover:border-gold-glow/30 transition-colors`}
            >
