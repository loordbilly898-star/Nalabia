...
const ChatbotView: React.FC<ChatbotViewProps> = ({
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    if ((!input.trim() && !selectedImage) || status !== ProcessingState.IDLE)
      return;
    ...
    try {
      ...
      try {
        ...
        logEvent("api", "Chatbot stream completed", {
          responseTime: Date.now() - startTime,
        });
      } catch (streamError: any) {
        logEvent("api", "Chatbot stream interrupted", {
          errorDetail: streamError.message,
        });
      ...
      let errorMessage =
        "Erro na IA. Tente reformular ou enviar novamente em alguns segundos.";
      if (typeof error?.message === "string") {
        if (
          ...
          errorMessage =
            "A IA está sobrecarregada ou atingiu o limite de uso. Tente novamente em 2 minutos.";
  ...
  const renderMentorMessage = (content: string) => {
    const sections = [
      ...
      {
        key: "[TRANSCRIÇÃO OBRIGATÓRIA]",
        label: "Análise Visual Estrita",
        icon: <ScanFace size={12} />,
        color: "text-gray-400",
      },
      {
        key: "[LEITURA]",
        label: "Leitura do Momento",
      ...
      <div className="space-y-4">
        {parts.map((p, i) => (
          <div key={i} className="space-y-1.5">
            <div
              className={`flex items-center gap-1.5 text-[10px] uppercase font-mono tracking-widest ${p.color}`}
            >
              {p.icon}
              {p.label}
            </div>
            <div
              className={`text-sm leading-relaxed ${p.label === "Sugestão de Resposta" || p.label === "Resposta Recomendada" ? "bg-gold-glow/5 border border-gold-glow/20 rounded-lg p-3 font-medium text-gold-glow" : "text-gray-300"}`}
            >
              {p.text === "NÃO RESPONDA" ? (
    ...
    <div className="flex flex-col h-full">
      <div className={`p-4 border-b border-gold-dim/10 ${getThemeHeaderBg()}`}>
        ...
        <p className="text-xs text-gray-500 mt-1">
          Estratégia, leitura fria e domínio social.
        </p>
      </div>
          ...
          <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
            ...
            <div>
              <h3 className="text-sm font-bold text-gray-300">
                Qual a missão de hoje, parceiro?
              </h3>
              <p className="text-xs text-gray-500 mt-2 max-w-xs mx-auto">
                Diz aí o que está rolando. Print, dúvida ou estratégia? Estou
                aqui para garantir que você não erre o frame.
              </p>
            </div>
          </div>
        ) : (
              ...
              >
                {msg.image && (
                  <img
                    src={msg.image}
                    alt="Upload"
                    className="max-w-full rounded-lg mb-2 border border-gray-800"
                  />
                )}
                {msg.role === "assistant" ? (
                  renderMentorMessage(
        ...
        {selectedImage && (
          <div className="mb-3 relative inline-block">
            <img
              src={selectedImage}
              alt="Preview"
              className="h-16 w-16 object-cover rounded-lg border border-gold-dim/10"
            />
            <button
