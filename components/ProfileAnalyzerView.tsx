...
const ProfileAnalyzerView: React.FC<ProfileAnalyzerViewProps> = ({
  settings,
  onAddProfile,
}) => {
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400">
            Envie um print do perfil dela e receba a análise completa e os
            melhores abridores.
          </p>
        </div>
      </div>
          ...
          <div
            className={`relative border-2 border-dashed rounded-2xl overflow-hidden transition-colors flex flex-col items-center justify-center min-h-[300px] ${
              selectedImages.length > 0
                ...
                : `border-gold-dim/20 ${getThemeInputBg().split(" ")[0]} hover:border-gold-glow/50 cursor-pointer`
            }`}
            onClick={() =>
            ...
            {selectedImages.length > 0 ? (
              ...
              <div className="text-center p-6">
                ...
                <p className="text-gray-300 font-medium mb-2">
                  Toque para enviar prints
                </p>
                <p className="text-gray-500 text-sm">
                  Tinder, Bumble, Instagram (Fotos + Bio)
                </p>
              </div>
            )}
            <input
        ...
        <div className="flex flex-col gap-4">
          {analysisResult ? (
            <div className="space-y-6 animate-fade-in">
              ...
              {/* Vibe */}
              <div
                className={`${getThemeInputBg().split(" ")[0]} border border-gold-dim/10 rounded-xl p-5`}
              >
                <h3 className="text-xs font-mono text-gold-glow uppercase tracking-widest mb-2">
                  Vibe Detectada
                </h3>
                <p className="text-gray-200 text-sm leading-relaxed">
                  {typeof analysisResult.vibe === "string"
                    ? analysisResult.vibe
                    : JSON.stringify(analysisResult.vibe)}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-emerald-950/20 border border-emerald-900/30 rounded-xl p-4">
                  <h3 className="text-[10px] font-mono text-emerald-500 uppercase tracking-widest mb-3">
                    Green Flags
                  </h3>
                ...
                <div className="bg-rose-950/20 border border-rose-900/30 rounded-xl p-4">
                  <h3 className="text-[10px] font-mono text-rose-500 uppercase tracking-widest mb-3">
                    Red Flags
                  </h3>
                  <ul className="space-y-2">
              ...
              <div
                className={`${getThemeInputBg().split(" ")[0]} border border-gold-dim/10 rounded-xl p-5`}
              >
                <h3 className="text-xs font-mono text-gold-glow uppercase tracking-widest mb-4">
                  Abridores Sugeridos
                </h3>
                <div className="space-y-3">
                  {Array.isArray(analysisResult.icebreakers) &&
                    analysisResult.icebreakers.map((icebreaker, i) => (
                      <div
                        key={i}
                        className={`${getThemeInputBg().split(" ")[0]} border border-gold-dim/10 rounded-lg p-3 flex items-start justify-between group hover:border-gold-glow/30 transition-colors`}
                      >
            ...
            >
              ...
              <p className="text-gray-500 max-w-sm text-sm">
                Faça o upload de um print para a IA analisar a personalidade e
                gerar abridores personalizados.
              </p>
            </div>
          )}
        </div>
