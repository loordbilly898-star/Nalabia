  ...
  return (
    <div className="flex flex-col h-full animate-fade-in p-4 pb-20 overflow-y-auto">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-sm font-mono font-bold tracking-[0.2em] uppercase text-nalabia-gold">
          Perfis & Memória
        </h2>
        <button
      ...
      {isAdding && (
        <div
          className={`${getThemeInputBg().split(" ")[0]} p-4 rounded-lg border border-nalabia-gold/50 mb-6 animate-fade-in`}
        >
          <h3 className="text-xs font-mono uppercase text-gray-400 mb-3">
            Novo Perfil
          </h3>
          <input
            type="text"
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              onClick={handleAdd}
              className="bg-nalabia-gold text-black text-xs font-bold px-4 py-2 rounded"
            >
              Criar Perfil
            </button>
          </div>
        ...
        {profiles.map((profile) => (
          ...
          >
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3
                      className={`font-bold text-sm ${activeProfileId === profile.id ? "text-white" : "text-gray-300"}`}
                    >
                      {typeof profile.name === "string"
                        ? profile.name
                    ...
                    <div className="flex items-center gap-1 px-1.5 py-0.5 bg-nalabia-gold/10 border border-nalabia-gold/20 rounded text-[9px] text-nalabia-gold font-mono">
                      ...
                      <span>
                        {memories?.find((m) => m.id === profile.id)
              ...
              {activeProfileId === profile.id && (
                <Check size={16} className="text-nalabia-gold" />
              )}
            </div>

            <div className="mt-4 flex items-center gap-4 border-t border-gray-800/50 pt-3">
              <div className="flex flex-col">
                <span className="text-[8px] text-gray-600 uppercase font-mono">
                  Interesse
                </span>
                <span
                  className={`text-[10px] font-bold ${
                    typeof profile.metrics.interest === "string" &&
                    profile.metrics.interest === "Alto"
                      ? "text-green-400"
                      : typeof profile.metrics.interest === "string" &&
                          profile.metrics.interest === "Baixo"
                        ? "text-red-400"
                        : "text-yellow-400"
                  }`}
                >
                  {typeof profile.metrics.interest === "string"
                    ? profile.metrics.interest
                    : JSON.stringify(profile.metrics.interest)}
                </span>
              </div>
              <div className="flex flex-col">
                <span className="text-[8px] text-gray-600 uppercase font-mono">
                ...
                <span className="text-[10px] text-gray-400 font-bold">
                  {typeof profile.metrics.risk === "string"
                    ? profile.metrics.risk
                    : profile.metrics.risk
                      ? JSON.stringify(profile.metrics.risk)
                      : "N/A"}
                </span>
              </div>
              <div className="flex flex-col">
            ...
            {/* Memory Display */}
            {memories &&
              memories.find((m) => m.id === profile.id)?.observations && (
                  ...
                  <ul className="space-y-1">
                    {memories
                      ...
                      ?.observations?.map((obs, idx) => (
                        <li
                          key={idx}
                          className="text-[10px] text-gray-400 pl-3 relative before:content-[''] before:absolute before:left-0 before:top-[5px] before:w-1 before:h-1 before:bg-blue-400/50 before:rounded-full"
                        >
                          {obs}
                        </li>
                      ))}
      ...
      <div className="mt-8 text-center px-6">
        <p className="text-[10px] text-gray-600 leading-relaxed">
          Cada perfil mantém uma memória isolada. A IA aprende o padrão de
          comportamento específico de cada pessoa para sugerir estratégias
