      ...
      <div className="flex items-center justify-between px-1">
        ...
        {onRegenerate && (
          ...
          >
            <RefreshCw
              size={10}
              className={`text-gray-500 group-hover:text-gold-glow ${isRegenerating ? "animate-spin" : ""}`}
            />
            <span className="text-[8px] font-mono text-gray-500 group-hover:text-gold-glow uppercase">
              Regerar
            </span>
          </button>
        )}
          ...
          >
            <div>
              <div className="flex justify-between items-start mb-3">
                <span className="text-[9px] font-mono text-gray-500 uppercase tracking-wider border border-white/5 px-2 py-0.5 rounded-full">
                  {typeof res.type === "string"
                    ? res.type
                    : JSON.stringify(res.type) || "Opção"}
                </span>
                      ...
                      handleSave(
                        typeof res.text === "string"
                          ...
                          : typeof res === "string"
                            ? res
                            : JSON.stringify(res),
                        typeof res.type === "string" ? res.type : "Geral",
                        idx,
                        e,
                      )
                    }
              ...
              {res.explanation && (
                <div className="mt-3 p-2.5 bg-obsidian-lighter/50 rounded-lg border border-gold-dim/10">
                  <p className="text-[10px] text-gold-glow/80 font-mono leading-relaxed">
                    <span className="font-bold text-gold-glow mr-1">
                      🧠 NaLábia:
                    </span>
                    {typeof res.explanation === "string"
                      ? res.explanation
                      : JSON.stringify(res.explanation)}
                  </p>
                </div>
              )}
            </div>
